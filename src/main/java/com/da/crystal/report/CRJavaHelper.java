 ...
 * @LastEditDate          : 2023-11-20 16:05:55                               *
...
public class CRJavaHelper {
   ...
   * // How to use a JNDI data source with the Crystal Reports Java SDK on
   * Tomcat https://userapps.support.sap.com/sap/support/knowledge/en/1343290
   * @throws ReportSDKException
   * @throws IOException
   */
  public static void changeDataSource(ReportClientDocument clientDoc,
                                      String username, String password,
                                      String connectionURL, String driverName,
                                      String jndiName, String fileName,
                                      String filePath) {
    try {
      ...
      // connection How to use a JNDI data source with the Crystal Reports Java
      // SDK on Tomcat JNDI name for Crystal Report must start with 'jdbc/'
      ...
      IConnectionInfo oldConnectionInfo =
          clientDoc.getDatabaseController().getConnectionInfos(null).get(0);
      PropertyBag oldPropertyBag = oldConnectionInfo.getAttributes();
      if (jndiName.isBlank() &&
          oldPropertyBag.getStringValue("Server Type").equals("JDBC (JNDI)") &&
          oldPropertyBag.getStringValue("Connection URL")
              .equals(connectionURL) &&
          oldPropertyBag.getStringValue("Database Class Name")
              .equals(driverName)) {
      ...
      for (int i = 0; i < tables.size(); i++) {
        ...
        // ⚠️⚠️⚠️ Must Update the table with new table, It's seems a bug of Crystal
        // Report
      ...
      IStrings subReportNames =
          clientDoc.getSubreportController().getSubreportNames();
      for (int subNum = 0; subNum < subReportNames.size(); subNum++) {
        String subReportName = subReportNames.getString(subNum);

        tables = clientDoc.getSubreportController()
        ...
        for (int i = 0; i < tables.size(); i++) {
          ...
          // ⚠️⚠️⚠️ Must Update the table with new table, It's seems a bug of
          // Crystal Report
          newTable =
              changeDataSource(table, newPropertyBag, username, password);
      ...
      // if modified, save it, so that could directly login, change data source
      // is slowly.
   ...
   * @throws ReportSDKException
   */
  public static ITable changeDataSource(ITable table, PropertyBag propertyBag,
                                        String username, String password) {
    if (log.isDebugEnabled()) {
      log.debug("{} {} {}", table.getName(), table.getAlias(),
                table.getQualifiedName());
    }
    // We set the Fully qualified name to the Table Alias to keep the method
    // generic This workflow may not work in all scenarios and should likely be
    // customized to work in the developer's specific situation. The end result
    // of this statement will be to strip the existing table of it's db specific
    // identifiers. For example Xtreme.dbo.Customer becomes just Customer
    // table.setQualifiedName(table.getAlias());

    // Change properties that are different from the original datasource
    // For example, if the table name has changed you will be required to change
    // it during this routine table.setQualifiedName(TABLE_NAME_QUALIFIER);

    // Change connection information properties
    IConnectionInfo connectionInfo = table.getConnectionInfo();
    if (log.isDebugEnabled()) {
      log.debug(table.getName() + ":" +
                connectionInfo.getAttributes().toString());
    }
    connectionInfo.setAttributes(propertyBag);

    // Set database username and password
    // NOTE: Even if the username and password properties do not change
    // when switching databases, the database password is ⚠️*not*⚠️ saved in the
    // report and must be set at runtime if the database is secured.
   ...
   * @param subReportName The name of the subReport. If tables in the main
   *     report is to be used, "" should be passed
   * @throws ReportSDKException
   */
  public static void passResultSet(ReportClientDocument clientDoc,
                                   java.sql.ResultSet rs, String tableAlias,
                                   String subReportName)
      throws ReportSDKException {
    if (subReportName == null || subReportName.isEmpty()) {
      clientDoc.getDatabaseController().setDataSource(rs, tableAlias,
                                                      tableAlias);
    } else {
      clientDoc.getSubreportController()
          .getSubreport(subReportName)
   ...
   * @throws ReportSDKException
   */
  public static void passPOJO(ReportClientDocument clientDoc,
    ...
    if (subReportName == null || subReportName.isEmpty()) {
      clientDoc.getDatabaseController().setDataSource(
          dataSet, Class.forName(className), tableAlias, tableAlias);
    } else {
      clientDoc.getSubreportController()
          .getSubreport(subReportName)
          .getDatabaseController()
          .setDataSource(dataSet, Class.forName(className), tableAlias,
                         tableAlias);
    }
  }

  /**
   * Passes a populated collection of a Java class to a Table object, ⚠️⚠️⚠️ only
  ...
  public static void passPOJO(ReportClientDocument clientDoc,
                              @SuppressWarnings("rawtypes") Collection dataSet,
                              String className, String subReportName)
      throws ReportSDKException, ClassNotFoundException {
    ...
    if (subReportName == null || subReportName.isEmpty()) {
      ...
      tableAlias = clientDoc.getSubreportController()
                       .getSubreport(subReportName)
                       .getDatabaseController()
                       .getDatabase()
                       .getTables()
   ...
   * @throws ReportSDKException
   */
  public static boolean isParameterExists(ReportClientDocument clientDoc,
                                          String parameterName)
      throws ReportSDKException {
    Fields<IParameterField> parameterFields =
        clientDoc.getDataDefinition().getParameterFields();
    for (int i = 0; i < parameterFields.size(); i++) {
      IParameterField paramToChange =
          (IParameterField)parameterFields.getField(i);
   ...
   * @param clientDoc     The reportClientDocument representing the report being
   *     used
   * @param reportName    The name of the subReport. If tables in the main
   *     report is to be used, "" should be passed
   * @param parameterName The name of the parameter
   * @param newValue      The new value of the parameter
   * @throws ReportSDKException
   */
  public static void
  setDiscreteParameterValue(ReportClientDocument clientDoc, String reportName,
                            String parameterName, Object newValue)
      throws ReportSDKException {
    ...
    if (reportName == null || reportName.isEmpty()) {
      ...
      dataDefController = clientDoc.getSubreportController()
                              .getSubreport(reportName)
                              .getDataDefController();
    }

    ParameterFieldDiscreteValue newDiscValue =
        new ParameterFieldDiscreteValue();
    newDiscValue.setValue(newValue);

    ParameterField paramField =
        (ParameterField)dataDefController.getDataDefinition()
            .getParameterFields()
            .findField(parameterName, FieldDisplayNameType.fieldName,
                       Locale.getDefault());
  ...
  public static void
  setDiscreteParameterValue(ReportClientDocument clientDoc, String reportName,
                            String parameterName, Object[] newValues)
      throws ReportSDKException {
    clientDoc.getDataDefController()
        .getParameterFieldController()
   ...
   * @throws ReportSDKException
   */
  public static void setRangeParameterValue(ReportClientDocument clientDoc,
                                            String reportName,
                                            String parameterName,
                                            Object beginValue, Object endValue)
      throws ReportSDKException {
    setRangeParameterValue(clientDoc, reportName, parameterName, beginValue,
                           RangeValueBoundType.inclusive, endValue,
   ...
   */
  public static void
  setRangeParameterValue(ReportClientDocument clientDoc, String reportName,
                         String parameterName, Object[] beginValues,
                         Object[] endValues) throws ReportSDKException {
    setRangeParameterValue(clientDoc, reportName, parameterName, beginValues,
                           RangeValueBoundType.inclusive, endValues,
                           RangeValueBoundType.inclusive);
  }
   ...
   *
   * @param clientDoc      The reportClientDocument representing the report
   *     being used
   * @param reportName     The name of the subReport. If tables in the main
   *     report is to be used, "" should be passed
   * @param parameterName  The name of the parameter
  ...
  public static void setRangeParameterValue(
      ReportClientDocument clientDoc, String reportName, String parameterName,
      Object beginValue, RangeValueBoundType lowerBoundType, Object endValue,
  ...
  setRangeParameterValue(ReportClientDocument clientDoc, String reportName,
    ...
    // endValues array
    ParameterFieldRangeValue[] newRangeValues =
        new ParameterFieldRangeValue[beginValues.length];
   ...
   * @throws IOException
   */
  public static void exportPDF(ReportClientDocument clientDoc,
    ...
    export(clientDoc, exportOptions, response, attachment, "application/pdf",
           "pdf");
  }
   ...
   */
  public static void exportPDF(ReportClientDocument clientDoc,
                               HttpServletResponse response, int startPage,
                               int endPage, boolean attachment)
      throws ReportSDKExceptionBase, IOException {
    // PDF export allows page range export. The following routine ensures
    // that the requested page range is valid
    PDFExportFormatOptions pdfOptions = new PDFExportFormatOptions();
   ...
   * @throws IOException
   */
  public static void exportRTF(ReportClientDocument clientDoc,
                               HttpServletResponse response, boolean attachment)
      throws ReportSDKExceptionBase, IOException {
   ...
   * @throws IOException
   */
  public static void exportMSWord(ReportClientDocument clientDoc,
    ...
    export(clientDoc, exportOptions, response, attachment, "application/word",
           "doc");
  }
   ...
   * @throws IOException
   */
  public static void exportRTFEditable(ReportClientDocument clientDoc,
                                       HttpServletResponse response,
                                       boolean attachment)
      throws ReportSDKExceptionBase, IOException {
    // RTF export allows page range export. The following routine ensures
    // that the requested page range is valid
    EditableRTFExportFormatOptions rtfOptions =
        new EditableRTFExportFormatOptions();
    ExportOptions exportOptions = new ExportOptions();
  ...
  public static void exportRTFEditable(ReportClientDocument clientDoc,
                                       HttpServletResponse response,
                                       ServletContext context, int startPage,
                                       int endPage, boolean attachment)
      throws ReportSDKExceptionBase, IOException {
    // RTF export allows page range export. The following routine ensures
    // that the requested page range is valid
  ...
  public static void exportExcelDataOnly(ReportClientDocument clientDoc,
                                         ...
                                         boolean attachment)
      throws ReportSDKExceptionBase, IOException {
    DataOnlyExcelExportFormatOptions excelOptions =
        new DataOnlyExcelExportFormatOptions();
    ExportOptions exportOptions = new ExportOptions();
    exportOptions.setExportFormatType(ReportExportFormat.recordToMSExcel);
    exportOptions.setFormatOptions(excelOptions);

    export(clientDoc, exportOptions, response, attachment, "application/excel",
           "xls");
  }
   ...
   * @throws IOException
   */
  public static void exportCSV(ReportClientDocument clientDoc,
                               HttpServletResponse response, boolean attachment)
      throws ReportSDKExceptionBase, IOException {
    CharacterSeparatedValuesExportFormatOptions csvOptions =
        new CharacterSeparatedValuesExportFormatOptions();
    csvOptions.setSeparator(",");
    csvOptions.setDelimiter("\n");
    ExportOptions exportOptions = new ExportOptions();
    exportOptions.setExportFormatType(
        ReportExportFormat.characterSeparatedValues);
   ...
   * @throws IOException
   */
  private static void export(ReportClientDocument clientDoc,
   ...
   * @throws ReportSDKException
   */
  public static void printToServer(ReportClientDocument clientDoc,
                                   String printerName)
      throws ReportSDKException {
   ...
   */
  public static void printToServer(ReportClientDocument clientDoc,
                                   String printerName, int startPage,
                                   int endPage) throws ReportSDKException {
    PrintReportOptions printOptions = new PrintReportOptions();
    // Note: Printer with the <printer name> below must already be configured.
    printOptions.setPrinterName(printerName);
    printOptions.setJobTitle("Sample Print Job from Crystal Reports.");
    ...
    PrintReportOptions.PageRange printPageRange =
        new PrintReportOptions.PageRange(startPage, endPage);
