 ...
 * @LastEditDate          : 2023-11-20 16:05:23                               *
...
public class ReportController {
  ...
  @Value("${rpt.datasource.url}") private String url;

  @Value("${rpt.datasource.driverClassName}") private String driverClassName;

  @Value("${rpt.datasource.username}") private String username;

  @Value("${rpt.datasource.password}") private String password;

  @Value("${rpt.datasource.jndiName}") private String jndiName;

  @GetMapping("/Report/*/*")
  public void handReportRequest(HttpServletRequest req,
    ...
    try {
      ...
      List<String> allowedFormat =
          Arrays.asList("pdf", "xls", "doc", "rtf", "csv");
      if (!allowedFormat.contains(format)) {
        resp.getWriter().write("<H1>Document format {" + format +
                               "} is not supported!</H1>");
        return;
      }
      ...
      if (!file.exists()) {
        resp.getWriter().write("<H1>Report template {" + report +
                               "} not found!</H1>");
        return;
      }
      ...
      CRJavaHelper.changeDataSource(clientDoc, username, password, url,
                                    driverClassName, jndiName, report + ".rpt",
                                    reportsPath);
      ...
      for (String param : reportParams) {
        ...
        if (!reqParams.containsKey(param)) {
          ...
          resp.getWriter().write("<H1>Report param [" + param +
                                 "] not found in request!</H1>");
          return;
        } else {
